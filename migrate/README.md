...
The script also clears i-MSCP's immutable flag for the duration and restores it
afterwards. i-MSCP sets `+i` on a customer's web directory when that domain has
`web_folder_protection = yes` — 2 of the 146 here, `godrone.co.nz` and
`verticalhorizon.co.nz`. Root cannot create a file in such a directory, so
rsync's atomic write-then-rename fails with

```
rsync: [receiver] mkstemp ".../.htpasswd.GyJPGn" failed: Operation not permitted (1)
```

and rsync exits 23. Only the domain root carries the flag — `htdocs` and the
rest do not — so it only ever affected `.htpasswd` and `.htgroup`, which sit
directly in it. Those two are generated by i-MSCP from the `htaccess_users`
and `htaccess_groups` tables, so the restored database is their real source
anyway.

Exit codes 23 and 24 are now reported rather than treated as fatal, and both
are worth reading: 23 means some files did not transfer, and 24 means files
vanished mid-run — which usually means a service is still running on aws1.
...
1. **`authmysqlrc` loses its markers.** courier-authlib 0.72.4 refuses a
   ...
   *"marker line not found ... probably forgot to run sysconftool"* and then
   rejecting every login — `authmysql: REJECT`, and with authmysql the only
...
all still working after a further full installer run.
